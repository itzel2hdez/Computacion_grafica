...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//model = glm::mat4(1.0); en jerarquía rompe la conexión
		...
		// Crear instancias para completar el brazo y  la cabina. 

		//articulación 4 extremo derecho del tercer brazo
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 2.5f, 0.0f));
		...
		glm::mat4 modelPiramide = model;
		model = glm::scale(model, glm::vec3(6.0f, 4.5f, 6.0f));
		...
		// Frontal izquierda (tecla 1)
		model = modelPiramide; // Hijo de la base
		model = glm::translate(model, glm::vec3(-3.5f, -2.2f, 3.5f));
