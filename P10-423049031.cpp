...
float giroAvion = 0;
float cicloX = 0, cicloY = 0, cicloG = 0;
float rotAlas = 0.0f;
float velocidadAleteo = 5.0f;
bool alasParaAbajo = true;
...
void animate(void)
{
	...
	if (play)
	{
		...
		if (alasParaAbajo) {
			rotAlas -= velocidadAleteo * deltaTime;
			if (rotAlas <= -45.0f) alasParaAbajo = false;
		}
		else {
			rotAlas += velocidadAleteo * deltaTime;
			if (rotAlas >= 45.0f) alasParaAbajo = true;
		}
	}
}
...
void guardaArchivo(void) {
	std::ofstream archivo("keyframes_nave.txt", std::ios::out);
	if (archivo.is_open()) {
		archivo << FrameIndex << std::endl; // Guardamos el total de frames creados
		for (int i = 0; i < FrameIndex; i++) {
			archivo << KeyFrame[i].movAvion_x << " "
				<< KeyFrame[i].movAvion_y << " "
				<< KeyFrame[i].giroAvion << std::endl;
		}
		archivo.close();
		printf("EXITO: Se ha creado 'keyframes_nave.txt' con %d frames.\n", FrameIndex);
	}
	else {
...
void leeArchivo(void) {
	std::ifstream archivo("keyframes_nave.txt", std::ios::in);
	if (archivo.is_open()) {
		archivo >> FrameIndex; // Cargamos cuántos frames hay que leer
		for (int i = 0; i < FrameIndex; i++) {
			archivo >> KeyFrame[i].movAvion_x;
			archivo >> KeyFrame[i].movAvion_y;
			archivo >> KeyFrame[i].giroAvion;
		}
		archivo.close();
		printf("EXITO: Se cargaron %d frames desde el archivo.\n", FrameIndex);
	}
	else {
		printf("ERROR: No se encontro un archivo previo.\n");
	}
}
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//nave
		float dirX_faro = (giroAvion == 0.0f) ? 1.0f : -1.0f;
		spotLights[2].SetFlash(posblackhawk, glm::vec3(0.0f, -1.0f, 0.0f));
...
void inputKeyframes(bool* keys)
{
	...
	// gestion de archivo txt
	if (keys[GLFW_KEY_K]) { guardaArchivo(); } // tecla k para escribir
	if (keys[GLFW_KEY_O]) { leeArchivo(); }   // Tecla o para leer


	// movimiento
	if (!play) {
		// Teclas 1 y 2 para el zig-zag de ida
		if (keys[GLFW_KEY_1] || keys[GLFW_KEY_2] || keys[GLFW_KEY_3] || keys[GLFW_KEY_4]) {
			if (cicloX < 1) {
				if (keys[GLFW_KEY_1]) { movAvion_x += 2.5f; movAvion_y += 2.5f; } // sube 
				if (keys[GLFW_KEY_2]) { movAvion_x += 2.5f; movAvion_y -= 2.5f; } // baja 
				if (keys[GLFW_KEY_3]) { movAvion_x -= 2.5f; movAvion_y += 2.5f; } // regreso - sube
				if (keys[GLFW_KEY_4]) { movAvion_x -= 2.5f; movAvion_y -= 2.5f; } // regreso - baja
				printf("Nave movida a: X=%.2f, Y=%.2f\n", movAvion_x, movAvion_y);
				cicloX = 1; 
			}
		}
		else { cicloX = 0; } 
		...
		// giro 
		if (keys[GLFW_KEY_5] || keys[GLFW_KEY_6]) {
			if (cicloG < 1) {
				if (keys[GLFW_KEY_5]) giroAvion = 180.0f; // mira hacia atras
				if (keys[GLFW_KEY_6]) giroAvion = 0.0f;   // ve hacia adelante
				printf("Giro preparado: %.2f grados. ¡Dale a L para guardar!\n", giroAvion);
				cicloG = 1;
			}
		}
		else { cicloG = 0; }
