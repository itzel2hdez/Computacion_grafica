...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	// --- CONTROL INDEPENDIENTE DE LLANTAS ---
	if (key == GLFW_KEY_1)
	{
		theWindow->rueda1 += 5.0f; // Gira rueda frontal izquierda
	}
	if (key == GLFW_KEY_2)
	{
		theWindow->rueda2 += 5.0f; // Gira rueda frontal derecha
	}
	if (key == GLFW_KEY_3)
	{
		theWindow->rueda3 += 5.0f; // Gira rueda trasera izquierda
	}
	if (key == GLFW_KEY_4)
	{
		theWindow->rueda4 += 5.0f; // Gira rueda trasera derecha
	}
