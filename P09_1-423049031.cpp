...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// animacion del fuego y de la maquina
		if (fuegoPrendido) {
			anguloAeolipile += 0.5f * deltaTime; // Giro de la máquina

			toffsetFuegoV += 0.005f * deltaTime; // movimiento de la textura del fuego
		...
		model = glm::rotate(model, anguloAeolipile * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		if (fuegoPrendido) {
			...
			model = glm::scale(model, glm::vec3(1.8f, 1.5f, 1.5f));
