...
// --- CUERPO ---
		...
		modelaux = model; // Base para hijos (cabeza, patas, cola)
		...
		model = glm::scale(model, glm::vec3(0.8f, 0.6f, 0.7f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		for (float i : {-0.7f, 0.7f}) {
			...
			model = glm::scale(model, glm::vec3(0.4f, 0.7f, 0.2f));
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			meshList[0]->RenderMeshGeometry(); // Cubo
		}

		// --- PATAS DELANTERAS (1 y 2 - Teclas F y G) ---
		for (float i : {-0.8f, 0.8f}) {
			// HOMBRO
			model = modelaux;
			model = glm::translate(model, glm::vec3(1.5f, -0.8f, i));
			model = glm::rotate(model, glm::radians(mainWindow.getarticulacion1()), glm::vec3(0.0f, 0.0f, 1.0f));
			glm::mat4 hombroDel = model;
		...
		// --- PATAS TRASERAS (3 y 4 - Teclas H y J) ---
		for (float z_pos : {-0.8f, 0.8f}) {
			// CADERA 
			model = modelaux;
			model = glm::translate(model, glm::vec3(-1.5f, -0.8f, z_pos));
			model = glm::rotate(model, glm::radians(mainWindow.getarticulacion3()), glm::vec3(0.0f, 0.0f, 1.0f));
			glm::mat4 caderaAux = model;
